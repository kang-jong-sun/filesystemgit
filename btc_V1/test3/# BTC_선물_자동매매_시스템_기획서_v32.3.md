# BTC/USDT 선물 자동매매 시스템 기획서 v32.3
## "EMA(75)/SMA(750) Low-MDD Trend System"
## 엔진 독립적 재현 가능한 완전 명세
...
MARGIN_PCT        = FIXED           # 고정 1000 USDT (비율 아님)
LEVERAGE          = 10
MARGIN_MODE       = 'ISOLATED'    # 격리마진 (FC 1건 발생)
DAILY_LOSS_LIMIT  = -0.20         # 일일 -20%
```

> **주의**: SMA(750)은 초기 749개 봉에서 NaN을 반환합니다. WARMUP_BARS=600이므로 실거래 시작 시점(index=600)에서도 SMA(750)은 아직 NaN입니다. SMA가 유효한 값을 갖는 첫 봉은 index=749입니다. 따라서 실제 첫 크로스 감지는 index=749 이후부터 가능합니다.
...
# 초기 period-1개는 NaN
# SMA(750): 첫 749개 봉(index 0~748)은 NaN
```

### 3.3 ADX(20) — pandas ewm(alpha=1/20)
...
### 3.4 RSI(11) — ewm(alpha=1/11)

```python
delta = close.diff()
gain  = delta.where(delta > 0, 0.0)
loss  = (-delta).where(delta < 0, 0.0)
avg_gain = gain.ewm(alpha=1/11, min_periods=11, adjust=False).mean()
avg_loss = loss.ewm(alpha=1/11, min_periods=11, adjust=False).mean()
...
# gap >= 0.2 이면 진입 허용
# 주의: slow_ma가 NaN이면 gap 계산 불가 → 진입 불가
```
...
상태 변수 초기화:
  ...
  epx = 0.0             # 진입가
  psz = 0.0             # 포지션 금액 (1000 * 10 = 10,000 USDT 고정)
  slp = 0.0             # 현재 SL가
  ton = False           # TSL 활성 여부
  thi = 0.0             # LONG 고점 추적
...
FOR i = 600 TO len(data)-1:
    ...
    IF pos == 0:
        ...
        IF watching != 0 AND i > ws:
            ...
            # ════ 진입! ════
            mg = 1000                            # 고정 마진 1000 USDT
            psz = 1000 * 10                 # 포지션 크기 = 10,000 USDT (고정) (금액)
...
주의: SMA(750)이 NaN인 봉에서는 크로스 감지가 불가능합니다.
      fast_ma[i] > slow_ma[i] 비교에서 NaN은 항상 False를 반환합니다.
```
...
□ 2. EMA(75): index=749 시점 값 확인
□ 3. SMA(750): index=749에서 첫 유효값 확인 (index 0~748은 NaN)
□ 4. SL 체크: TSL 미활성 시에만 작동하는지 확인
□ 5. TSL 활성: 고가/저가 기준 +12% 도달 시 활성
□ 6. TSL 청산: 종가 기준 확인
□ 7. 총 거래: 69건 (SL33 + TSL19 + REV17)
□ 8. 최종 잔액: $13,236,537 (±$132K 이내)
□ 9. MDD: 43.5% 확인
□ 10. PF: 3.2 확인
```
...
| RSI | RSI(11) 40~80 | RSI(14) 35~75 | 진입 시점 차이 |
...
```python
# ═══ MA 설정 ═══
FAST_MA  = EMA(close, 75)        # pandas ewm(span=75, adjust=False)
SLOW_MA  = SMA(close, 750)       # pandas rolling(750).mean()
# 주의: SMA(750)은 첫 749봉 NaN

# ═══ 공통 ═══
ADX      = ADX(20)               # ewm(alpha=1/20)
RSI      = RSI(11)               # ewm(alpha=1/11)
SL       = 3%                    # 가격 기준, 저가/고가 발동, SL가에 청산
TA       = 12%                   # 가격 기준, 고가/저가 활성화
TSL      = 9%                    # 고점 대비 가격 기준, 종가에 청산
MARGIN   = FIXED           # 고정 1000 USDT%, LEVERAGE = 10x
...
>
> $5,000 → **$13,236,537** (75개월, 69거래, PF 3.2, MDD 43.5%)
