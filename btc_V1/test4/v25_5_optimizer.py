"""
v25.5 Optimizer - 100,000%+ 수익률 목표
========================================
핵심 변경:
- 레버리지: 10x / 12x / 15x (동적 탐색)
- 마진: 20~30% (30% 이하)
- SL: 15x에서는 3~5% 타이트 (청산 방지)
- PF 5+, MDD <20%
- 목표: $5,000 → $5,000,000+ (100,000%+)

v25.2 Model A 분석: PF 10.15, 63거래, 40%마진, 10x → +166,670%
15x 레버리지 + 30%마진 = 4.5x 효과 (v25.2의 4.0x보다 큼)
→ 더 적은 거래로도 복리 효과 극대화 가능
"""
import pandas as pd
import numpy as np
...
def main():
    print("="*70, flush=True)
    print("v25.5 OPTIMIZER - TARGET 100,000%+ RETURN", flush=True)
    print("$5,000 | Lev 10-15x | Margin ≤30% | PF 5+ | MDD <20%", flush=True)
    print("="*70, flush=True)
    ...
    _ = run_backtest_v271(
        ...
        0.05,0.05,0.02,0,0,15,0.30,0.15,-0.20,0.0004,5000.0,6,1.0,1,6
    )
    ...
    # Cross configs - v25.2 검증 + 변형
    cross_configs = [
        ('30m','ema',3,'ema',200, '15m',20, '5m',14),   # v25.2 A (PF 10.15)
        ('30m','ema',3,'ema',150, '15m',20, '5m',14),
        ('30m','ema',3,'ema',100, '15m',20, '5m',14),
        ('30m','ema',5,'ema',200, '15m',20, '5m',14),
        ('30m','ema',5,'ema',150, '15m',20, '5m',14),
    ...
    # SL/Trail은 레버리지별 다르게 (15x에서 타이트)
    sl_trail_by_lev = {
        10: [(0.05,0.06,0.03),(0.07,0.07,0.03),(0.07,0.08,0.03),(0.07,0.10,0.04),(0.10,0.10,0.04)],
        12: [(0.04,0.05,0.025),(0.05,0.06,0.03),(0.05,0.07,0.03),(0.07,0.07,0.03),(0.07,0.08,0.03)],
        15: [(0.03,0.04,0.02),(0.03,0.05,0.025),(0.04,0.05,0.025),(0.04,0.06,0.03),(0.05,0.06,0.03),(0.05,0.07,0.03)],
    }
    ...
    rev_modes = [0, 1]  # 0=no REV, 1=strict REV
    min_bars = [1, 6, 12, 24]

    # Total combos
    total = 0
    for lev in leverages:
        total += len(cross_configs) * len(adx_mins) * len(rsi_ranges) * len(macd_keys) * len(ich_opts) * len(sl_trail_by_lev[lev]) * len(margins) * len(entry_delays) * len(entry_tols) * len(rev_modes) * len(min_bars)
    print(f"\n  Total combos: {total:,}", flush=True)
    ...
    for lev in leverages:
        ...
        print(f"\n  === Leverage {lev}x ({len(sl_trails)} SL/Trail configs) ===", flush=True)
    ...
    pf3=[r for r in all_results if r['pf']>=3.0]
    ...
    for i,r in enumerate(by_ret[:15]):
        print(f"  #{i+1}: {r['cross']} Lev:{r['leverage']}x ADX>{r['adx_min']} RSI:{r['rsi_min']}-{r['rsi_max']} ICH:{r['ichimoku']} "
              ...
              f"| Ret:{r['return_pct']:,.0f}% PF:{r['pf']:.2f} MDD:{r['mdd']:.1f}% T:{r['trades']} ${r['final_cap']:,.0f}", flush=True)

    print(f"\n  === Top 10 by PF (Ret>=1000%) ===", flush=True)
    good=[r for r in all_results if r['return_pct']>=1000]
    if good:
        by_pf=sorted(good, key=lambda x: x['pf'], reverse=True)
        for i,r in enumerate(by_pf[:10]):
            print(f"  #{i+1}: {r['cross']} Lev:{r['leverage']}x | Ret:{r['return_pct']:,.0f}% PF:{r['pf']:.2f} MDD:{r['mdd']:.1f}% T:{r['trades']}", flush=True)
    else:
        print("  No results with Ret>=1000%", flush=True)

    # Select models
    # A: 최고 수익률
    model_a = by_ret[0]
    # B: PF 최대 (Ret>=500%)
    decent=[r for r in all_results if r['return_pct']>=500]
    model_b = max(decent, key=lambda x: x['pf']/(x['mdd']+3)) if decent else max(all_results[:50], key=lambda x: x['pf']/(x['mdd']+3))
    # C: 최고 점수 (균형)
    model_c = all_results[0]

    models={'A':model_a,'B':model_b,'C':model_c}
    for name,m in models.items():
        print(f"\n  Model {name}: {m['cross']} Lev:{m['leverage']}x", flush=True)
    ...
    for name,m in models.items():
        ...
        print(f"      Min:${np.min(fcs):,.0f} Max:${np.max(fcs):,.0f} MinRet:{np.min(rets):,.0f}% MinPF:{np.min(pfs):.2f}", flush=True)

    # Save
    output={'version':'v25.5','initial_capital':5000,'margin_mode':'isolated',
            'total_tested':tested,
    ...
    def conv(o):
        ...
        return o
    path=os.path.join(os.path.dirname(os.path.abspath(__file__)),'v25_5_results.json')
    with open(path,'w',encoding='utf-8') as f:
        json.dump(output,f,indent=2,default=conv,ensure_ascii=False)
    print(f"\nSaved: {path}\nTotal: {tested:,}\n{'='*70}\nv25.5 COMPLETE", flush=True)
