# BTC 10m Filter OFF test - various MA combos
...
print(f"BTC 10m: {len(df10)} bars")

cs = df10['close']; c = cs.values.astype(np.float64)
h = df10['high'].values.astype(np.float64); l = df10['low'].values.astype(np.float64)

mas = {}
for p in [100, 150, 200, 250, 300, 400, 500, 600, 900, 1200, 1500, 1575, 1800, 2000]:
    mas[p] = ema_pd(cs, p).values.astype(np.float64)

print()
print("=" * 95)
print("  BTC 10m Filter OFF — All MA Grid (20% Margin, 10x, SL2/TA54/TSL8)")
print("=" * 95)
print(f"  {'Strategy':>45} {'Final':>12} {'Ret%':>8} {'PF':>6} {'MDD':>6} {'T':>4} {'SL':>4} {'TSL':>4} {'WLR':>6}")
print("  " + "-" * 92)

tests = [
    ("EMA(300)/EMA(1800) [30m 100/600 등가]", 300, 1800),
    ("EMA(250)/EMA(1575) [ETH V8.16 동일]", 250, 1575),
    ("EMA(250)/EMA(1500)", 250, 1500),
    ("EMA(200)/EMA(1200)", 200, 1200),
    ("EMA(200)/EMA(1500)", 200, 1500),
    ("EMA(150)/EMA(900)", 150, 900),
    ("EMA(150)/EMA(1200)", 150, 1200),
    ("EMA(100)/EMA(600)", 100, 600),
    ("EMA(100)/EMA(900)", 100, 900),
    ("EMA(100)/EMA(1200)", 100, 1200),
    ("EMA(300)/EMA(2000)", 300, 2000),
    ("EMA(400)/EMA(2000)", 400, 2000),
    ("EMA(500)/EMA(2000)", 500, 2000),
]

for label, fp, sp in tests:
    r = bt(c, h, l, mas[fp], mas[sp], len(c), 2.0, 54.0, 8.0, 18, True, sp, 0.20)
